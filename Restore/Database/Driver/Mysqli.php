 ...
 */
namespace Akeeba\Engine\Restore\Database\Driver;

use Akeeba\Engine\Restore\Database\Driver;
...
class Mysqli extends Driver
{
	...
	public function __construct($options)
	{
		...
		$defaultAllowUtf8Mb4 = defined('ANGIE_ALLOW_UTF8MB4_DEFAULT') ? ANGIE_ALLOW_UTF8MB4_DEFAULT : true;
	...
	public function connect()
	{
		...
		if (!empty($tmp))
		{
			...
			// Extract the host name only
			$this->options['host'] =
				substr($this->options['host'], 0, strlen($this->options['host']) - (strlen($tmp) + 1));
		...
		@mysqli_query($this->connection, "SET GLOBAL max_allowed_packet=67108864;");
		@mysqli_query($this->connection, "SET SESSION max_allowed_packet=67108864;");
