...
use Akeeba\Engine\Restore\Container\Container;
use Akeeba\Engine\Restore\Exception\ExceptionApp;
use Akeeba\Engine\Restore\Timer\Timer;
use Exception;
...
abstract class Restore
{
	 ...
	 * @throws ExceptionApp
	 */
	public function __construct($dbkey, $dbjsonValues, Container $container = null)
	{
		...
		$this->logFile             = storage_path('restore/' . $this->dbjsonValues['failed_query_log']);
		...
		$this->timer               = new Timer(0, (int) $this->dbjsonValues['maxexectime'], (int) $this->dbjsonValues['runtimebias']);
	}
	...
	{
		if (is_object($this->db) && ($this->db instanceof Driver))
		{
			try
			{
				$this->db->disconnect();
	 ...
	 *
	 * @staticvar  array  $instances  The array of Restore instances
	 *
	 ...
	 * @throws Exception
	 */
	public static function getInstance($dbkey, $dbjsonValues = null, Container $container = null)
	{
		...
		if (!array_key_exists($dbkey, $instances))
		{
			...
			{
				throw new Exception(trans('ANGI_RESTORE_ERROR_UNKNOWNKEY', $dbkey));
			}
			...
			$class = 'Akeeba\Engine\Restore\Database\Restore\\' . ucfirst($dbjsonValues['dbtype']);
   ...
   protected function getNextFile()
   {
	...
	protected function openFile()
	{
		...
		{
			throw new Exception(trans('ANGI_RESTORE_ERROR_CANTOPENDUMPFILE', $this->filename));
		}
		else
		{
			// Get the file size
			...
			{
				throw new Exception(trans('ANGI_RESTORE_ERROR_UNKNOWNFILESIZE'));
			}
		}
		...
		{
			throw new Exception(trans('ANGI_RESTORE_ERROR_INVALIDPARAMETERS'));
		}
		...
		{
			throw new Exception(trans('ANGI_RESTORE_ERROR_AFTEREOF'));
		}

		// Set file pointer to $foffset
		if (fseek($this->file, $this->foffset) != 0)
		{
			throw new Exception(trans('ANGI_RESTORE_ERROR_CANTSETOFFSET'));
		}
	...
	protected function getDatabase()
	{
		if (!is_object($this->db))
		{
			...
			$class = 'Akeeba\Engine\Restore\Database\Driver\\' . ucfirst(strtolower($options['driver']));
	...
	protected function readNextLine()
	{
		...
		while (!feof($this->file) && (strpos($query, $this->marker) === false))
		{
	...
	public function stepRestoration()
	{
		...
		if ($current_foffset === false)
		{
			...
			throw new Exception(trans('ANGI_RESTORE_ERROR_CANTREADPOINTER'));
		}
	...
	private function handleFailedQuery($sql, $exc)
	{
		...
		$message = '<h2>' . trans('ANGI_RESTORE_ERROR_ERRORATLINE', $this->linenumber) . '</h2>' . "\n";
		$message .= '<p>' . trans('ANGI_RESTORE_ERROR_MYSQLERROR') . '</p>' . "\n";
		$message .= '<tt>ErrNo #' . htmlspecialchars($exc->getCode()) . '</tt>' . "\n";
		$message .= '<pre>' . htmlspecialchars($exc->getMessage()) . '</pre>' . "\n";
		$message .= '<p>' . trans('ANGI_RESTORE_ERROR_RAWQUERY') . '</p>' . "\n";
