 ...
 */
namespace Akeeba\Engine\Restore\Session;

use Akeeba\Engine\Restore\Utils\Ip;
...
class Session
{
	...
	public function __construct()
	{
		...
		if (defined('ANGIE_FORCED_SESSION_KEY') && ANGIE_FORCED_SESSION_KEY)
		{
			...
			{
				die(trans('SESSIONBLOCKED_HEADER_IN_USE'));
			}
		}

		$storagefile       = storage_path('app/temp/restore/storagedata-' . $this->sessionkey . '.dat');
		...
		if (self::ENABLE_EXTRA_SECURITY)
		{
			try
			{
				...
				$di = new DirectoryIterator(storage_path('app/temp/restore'));
