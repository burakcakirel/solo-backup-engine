 ...
 */
namespace Akeeba\Engine\Restore\Filter;
...
{
	/**
	 * @var    array  A container for Input instances.
	 */
	...
	public static function &getInstance($tagsArray = array(), $attrArray = array(), $tagsMethod = 0, $attrMethod = 0, $xssAuto = 1)
	{
		...
		{
			self::$instances[$sig] = new Input($tagsArray, $attrArray, $tagsMethod, $attrMethod, $xssAuto);
		}
	...
	protected function _decode($source)
	{
		...
		// Convert decimal
		$source = preg_replace_callback('/&#(\d+);/m', function($matches){ return utf8_encode(chr($matches[0]));}, $source); // decimal notation

		// Convert hex
		$source = preg_replace_callback('/&#x([a-f0-9]+);/mi', function($matches){ return utf8_encode(chr('0x'.$matches[0]));}, $source); // hex notation
