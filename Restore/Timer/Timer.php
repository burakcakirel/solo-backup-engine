 ...
 */
namespace Akeeba\Engine\Restore\Timer;
...
class Timer
{
	...
	{
		logger(__METHOD__ . '(' . $max_exec_time . ', ' . $runtime_bias . ')');
	...
	public function enforce_min_exec_time($log = true, $serverSideSleep = true)
	{
		...
		if (($minexectime > $elapsed_time) && ($elapsed_time > 0))
		{
			...
			if (!$serverSideSleep)
			{
				logger("Asking client to sleep for $sleep_msec msec");
			...
			elseif (function_exists('usleep'))
			{
				if ($log)
				{
					logger("Sleeping for $sleep_msec msec, using usleep()");
				}
			...
			elseif (function_exists('time_nanosleep'))
			{
				if ($log)
				{
					logger("Sleeping for $sleep_msec msec, using time_nanosleep()");
				}
			...
			elseif (function_exists('time_sleep_until'))
			{
				if ($log)
				{
					logger("Sleeping for $sleep_msec msec, using time_sleep_until()");
				}
			...
			elseif (function_exists('sleep'))
			{
				...
				if ($log)
				{
					logger("Sleeping for $sleep_sec seconds, using sleep()");
				}
		...
		elseif ($elapsed_time > 0)
		{
			...
			if ($log)
			{
				logger("No need to sleep; execution time: $elapsed_time msec; min. exec. time: $minexectime msec");
			}
		}
